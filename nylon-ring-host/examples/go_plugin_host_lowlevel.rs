// Example demonstrating loading and using a low-level Go plugin with NylonRingHost
...
async fn main() -> Result<(), NylonRingHostError> {
    // Determine the path to the Go plugin library (low-level)
    ...
    for ext in &extensions {
        ...
        test_path.push(format!("nylon_ring_go_plugin.{}", ext));
        if test_path.exists() {
    ...
    if !found {
        ...
        plugin_path.push(format!("nylon_ring_go_plugin.{}", extensions[0]));
    }

    println!("Loading Go plugin (low-level) from: {:?}", plugin_path);
    ...
    println!("✓ Go plugin (low-level) loaded successfully!");
